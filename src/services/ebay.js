...
export async function getEbayToken() {
  ...
  console.log('eBay OAuth — tentative...');
  ...
  console.log('eBay OAuth status:', res.status, data.error || 'OK');
...
export async function searchActiveListings(player, set, condition, sport) {
  ...
  const sportKeyword = sport === 'NHL' ? 'hockey card' : sport === 'NFL' ? 'football card' : sport === 'NBA' ? 'basketball card' : sport === 'MLB' ? 'baseball card' : 'sports card';
  const q = encodeURIComponent(`${player} ${set} ${condition} ${sportKeyword}`);
  const url = `https://api.ebay.com/buy/browse/v1/item_summary/search?q=${q}&category_ids=${SPORTS_CARDS_CATEGORY}&limit=10&sort=price&filter=buyingOptions%3A%7BFIXED_PRICE%7D`;
  ...
  if (!res.ok) { console.error('Browse error:', res.status); throw new Error(`Browse API error: ${res.status}`); }
...
export function computeStats(listings, cadUsdRate = 0.74) {
  ...
  const cadPrices = listings.map(l => l.currency === 'USD' ? l.price / cadUsdRate : l.price);
  return { avg: cadPrices.reduce((s, p) => s + p, 0) / cadPrices.length, low: Math.min(...cadPrices), high: Math.max(...cadPrices), count: listings.length };
}
